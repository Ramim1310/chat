...
function Sidebar({ users, joinRoom, activeRoom, currentUser, onUserClick, onLogout, onTabChange, onFriendAction, initialTab = 'chats' }) {
  const [activeTab, setActiveTab] = useState(initialTab); // chats, search, requests, community
    ...
    <div className="w-1/3 md:w-1/4 h-full glass border-r border-white/10 flex flex-col relative z-20">
      ...
      <div className="flex p-2 gap-1 bg-black/20 m-4 rounded-xl">
          <button 
            ...
            className={`flex-1 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 ${activeTab === 'chats' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Chats
          </button>
          <button 
            onClick={() => handleTabChange('community')}
            className={`flex-1 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 ${activeTab === 'community' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Forum
          </button>
          <button 
            onClick={() => handleTabChange('search')}
            className={`flex-1 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 ${activeTab === 'search' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Search
          </button>
          <button 
            onClick={() => handleTabChange('requests')}
            className={`flex-1 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 ${activeTab === 'requests' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/30' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
          >
            Requests
          </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 custom-scrollbar">
        ...
        {activeTab === 'community' && (
            <div className="p-4 rounded-xl border border-dashed border-emerald-500/30 text-center bg-emerald-500/10">
                <p className="text-sm text-emerald-400">Welcome to the Forum Feed.</p>
                <p className="text-xs text-slate-400 mt-2">Posts wait for you on the right side.</p>
            </div>
        )}
